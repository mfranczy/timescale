...
func Parse() (Args, error) {
	...
	flag.IntVar(&a.WorkersNum, "workers-num", 1, "Number of running workers between <1-100>")
	flag.StringVar(&a.DBConfig, "db-config", "/etc/timescale-cli/db.yaml", "Path to database config file")
	flag.Parse()
	...
	if a.WorkersNum < 1 || a.WorkersNum > 100 {
		return a, fmt.Errorf("workers-num parameter has to be in <1-100>")
	}
